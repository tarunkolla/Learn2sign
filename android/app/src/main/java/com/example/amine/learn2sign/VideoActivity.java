...
public class VideoActivity extends Activity implements SurfaceHolder.Callback {
    ...
    private void initRecorder(Surface surface) throws IOException {
        ...
        String mode = "";
        switch(bundle.get("recordButtonId").toString()){
            case "bt_record":
                mode = "learn";
            case "bt_record_practice":
                mode = "practice";

        }
        File file = new File(Environment.getExternalStorageDirectory().getPath() + "/Learn2Sign/"
                + sharedPreferences.getString(INTENT_ID,"0000")+"_"+mode+"_"+word+"_0" + ".mp4");
        while(file.exists()) {
            i++;
            file = new File(Environment.getExternalStorageDirectory().getPath() + "/Learn2Sign/"
                    + sharedPreferences.getString(INTENT_ID,"0000")+"_"+mode+"_"+word+"_"+i + ".mp4");
        }

        //add i and map it to the word. if each contans atleast 3 vids enable practice.
